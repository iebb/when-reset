...
Provider credentials are write-only through the Worker API: the app can upload them after explicit consent, but no response or sync route returns plaintext credentials, encrypted credential envelopes, or credential fingerprints. The app ignores a `credentials` field even if a modified server tries to send one. Credentials are encrypted with AES-256-GCM before D1 storage. The authenticated encryption context binds ciphertext to its device, account, provider, and schema version. D1 also encrypts its storage at rest. This protects a copied database, but the Worker must be able to decrypt credentials internally to call providers: anyone controlling the Worker deployment or `CREDENTIAL_ENCRYPTION_KEY` is inside the trust boundary. Use only a Worker account you control. Requests use fixed provider HTTPS endpoints, including Synthetic and Warp, response sizes are bounded, and structured logs never contain credentials, provider response bodies, account IDs, device IDs, or quota values. Ollama Cloud sessions, Antigravity OAuth, and compatible custom endpoints are intentionally on-device only.
